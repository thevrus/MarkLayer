...
- **OG cards**: hand-written SVG rasterized by `@resvg/resvg-wasm`, cached in R2. The card draws the person's own annotation geometry onto a panel scaled from `captureViewport`, so every share preview is its real data — `og-marks.ts` parses and sanitizes the ops (colors are reparsed into `#rrggbb`; ops are stored user input), `og-card.ts` composes the SVG and is pure so it can be rendered offline, `og.ts` owns the wasm and the favicon fetch. Geist is bundled as base64 TTFs (`og-fonts.ts`, regenerate with `scripts/build-og-fonts.py`) — the previous runtime fetch from the Google Fonts CSS API broke silently when Google switched that response to `.woff`. Disabled for localhost / private hosts.
